...
//
// Example:
// - anvelocom_admin_display_submenu_page("Comenzi", "orders", new Orders_Table(), true, true)
function anvelocom_admin_display_submenu_page($title, $page, $table, $addable, $searchable, $editable) {
  ...
  if (($_POST) && ($_POST['action'] == 'submit-form')) {
    ...
    echo anvelocom_admin_div($table->parent_id, $form_url, 'save');
    
    echo anvelocom_admin_form_save($_POST, $table->get_editables(), $page, $nonce, $table);
  }
  ...
  if ($editable) {
    if ( (isset($_REQUEST['action'])) && ($_REQUEST['action'] == 'edit') && ($_REQUEST['page'] == $form_url)) {
      // ------------------------------------------------
    	...
    	// Display the main div
    	echo anvelocom_admin_div($table->parent_id, $form_url, 'edit');
      
      // Get which item to edit or create an empty one
      $item = anvelocom_admin_form_setup($_REQUEST[$page], $page);
      
      // Page title
      echo "<h2>" . $item->page_title . " " . $title . "</h2>";
      
      // Display the form 
      echo anvelocom_admin_form_body($item, $table, $nonce);
      
      
      // Display detail tables
      foreach ($table->get_detail_tables($item->data['id']) as $detail) {
      	anvelocom_admin_display_detail($detail);
      }
      ...
      // ------------------------------------------------
    	...
    	// Display the main div
    	echo anvelocom_admin_div($table->parent_id, $form_url, 'list');
...
function anvelocom_admin_div($parent, $page, $action) {
...
//
function anvelocom_admin_display_detail($detail) {
	$page = "anvelocom-" . $detail['page'];
  if ($_REQUEST['page'] != $page) {
    anvelocom_admin_display_submenu_page($detail['title'], $detail['page'], $detail['table'], true, true, true);
  }
}
...
function anvelocom_admin_form_save($post, $editables, $table_name, $nonce, $table) {
	$msg = '';
	...
	if (wp_verify_nonce( $post['nonce'], $nonce )) {
      ...
      $table_name2 = $wpdb->prefix . "anvelocom_" . $table_name;
      
      $required = anvelocom_admin_form_check_required_fields_for_save($post, $editables);
      if ($required == "") {
...
function anvelocom_admin_form_check_required_fields_for_save($post, $editables) {
	$ret == "";
...
function anvelocom_admin_form_body($item, $table, $nonce) {
	// Get the fields to edit
	$editables = $table->get_editables($item); ?>
	<form id="edit" action="" method="post">
    <table class="form-table">
      <tbody>
        <?php foreach ($editables as $field) { echo anvelocom_admin_form_field($field, $item, $table); } ?>
      </tbody>
    </table>
...
//
function anvelocom_admin_form_field($field, $item, $table) {
...
function anvelocom_admin_form_setup($request, $table_name) {
  $id = $request;
  
  global $wpdb;
  $table = $wpdb->prefix . "anvelocom_" . $table_name;
